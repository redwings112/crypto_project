...
import { initialState, useListState, useListStateReady } from ".";
import { useActiveListUrls, useAllLists } from "./hooks";

const EXCHANGE_PAGE_PATHS = ["liquidity"];
...
export default function Updater(): null {
  ...
  useQuery(
    (isReady && ["first-fetch-token-list", lists]) || [],
    () => {
      return Promise.all(
  ...
  useQuery(
    includeListUpdater && isReady && listState !== initialState
      ? ["token-list"]
      : [],
    async () => {
      return Promise.all(
